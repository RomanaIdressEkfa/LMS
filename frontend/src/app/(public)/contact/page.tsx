...
const CHANNELS = [
  { icon: "✉️", title: "Email", value: "support@lms.test", grad: "grad-primary" },
  { icon: "💬", title: "Live chat", value: "Mon–Fri, 9am–6pm", grad: "grad-purple" },
  { icon: "📍", title: "Office", value: "Remote-first, worldwide", grad: "grad-sunset" },
];
    ...
    <div>
      <section className="grad-hero border-b border-[var(--border)]">
        <div className="mx-auto max-w-4xl px-5 py-14 text-center md:px-8">
          <h1 className="text-4xl md:text-5xl">Get in <span className="gradient-text">touch</span></h1>
          <p className="mx-auto mt-3 max-w-xl text-lg font-semibold text-[var(--muted)]">Questions, feedback or partnership ideas? We&apos;d love to hear from you.</p>
        </div>
      </section>
          ...
          {CHANNELS.map((c) => (
            <div key={c.title} className="card flex items-center gap-4 p-5">
              ...
              <div>
                ...
                <p className="text-sm font-semibold text-[var(--muted)]">{c.value}</p>
              </div>
            </div>
          ))}
        </div>
